...
-- Award winnings
CREATE OR REPLACE FUNCTION award_win(p_user_id INT,p_amount NUMERIC,p_game_id INT) RETURNS NUMERIC AS $$DECLARE v_new_win NUMERIC; v_total NUMERIC;BEGIN  UPDATE users  SET win_balance=COALESCE(win_balance,0)+p_amount,      balance=COALESCE(bonus_balance,0)+COALESCE(win_balance,0)+p_amount,      total_wins=total_wins+1,      total_winnings=total_winnings+p_amount  WHERE id=p_user_id  RETURNING win_balance,balance INTO v_new_win,v_total;  INSERT INTO transactions(user_id,type,amount,balance_after,reference)  VALUES(p_user_id,'win',p_amount,v_total,p_game_id::TEXT);  RETURN v_total;END;$$ LANGUAGE plpgsql;
